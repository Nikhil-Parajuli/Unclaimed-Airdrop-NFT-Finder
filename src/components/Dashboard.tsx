...
export function Dashboard() {
  ...
  const filterButtons: Array<{ status: FilterStatus; count: number }> = [
    { status: 'unclaimed', count: displayedAssets.length },
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex space-x-2">
              {filterButtons.map(({ status, count }) => (
                <button
                  key={`filter-${status}`}
          ...
          <div className="space-y-3">
            {filterStatus === 'unclaimed' ? (
              displayedAssets.map((asset, index) => (
                <AssetCard 
                  key={`asset-${index}-${asset.id || ''}-${asset.chain || ''}`}
                  asset={asset}
